...
export type ReviewData={barcode:string;productId:string;current:{name:string|null;brand:string|null;packaging:{key:string;role:string;material:string;form:string;guidance?:{container?:{color:string};status:string}}[];publication:unknown};packet:null|{current:{name:string|null;brand:string|null;packaging:{key:string;role:string;material:string;form:string}[]};sources:{id:string;url:string;identityExcerpt:string;trustReason:string;independentlyVerified:boolean}[];proposals:{field:string;value:string;sourceId:string}[];inspectedPhotoHashes:string[]};outcome:ProductOutcome|null;images:{url:string;authenticated:boolean}[];stale:boolean;revision:string;readOnly:true;enrichment:false;actions:[]};
...
export const reviewLabels={en:{ready:'Ready for review',identity:'Identity conflict',material:'Missing material evidence',photo:'Photo rights / quality unresolved',published:'Already published',none:'No evidence',failed:'Research failed'},es:{ready:'Listo para revisar',identity:'Conflicto de identidad',material:'Falta evidencia del material',photo:'Derechos / calidad de foto pendientes',published:'Ya publicado',none:'Sin evidencia',failed:'Investigación fallida'}};
export function compactCounts(c:Counts){return {processed:c.attempted,updated:c.published,needsReview:c.deferred+c.staged,failed:c.failed,unknown:c.attemptedUnknown||0};}
