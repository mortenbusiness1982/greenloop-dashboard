...
Technical-review regressions: a20-second detail timeout releases a hung request, keeps last reviewed data, supports retry and explicit replacement, and ignores late results after unmount. Both staged and proposed_not_published map to Ready for review. Pending-only API photo changes are tested while catalogue metadata is held constant in an isolated fixture. Optional adapter export failures no longer affect core report acknowledgements and retry only the exact saved packet.
