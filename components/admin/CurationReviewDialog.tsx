...
type ManualPermission={expectedRevision:string;evidenceFingerprint:string;packetDigest:string|null;rejected:boolean;completed?:boolean;disposition?:string|null;detailFields?:('name'|'brand')[];metadataFields?:('name'|'brand')[];packagingKeys?:string[];confirmedPackagingKeys?:string[]};
...
export function CurationReviewDialog({target,language,onClose}:{target:ReviewTarget;language:'en'|'es';onClose:()=>void}){
 ...
 const [selectedDetails,setSelectedDetails]=useState<('name'|'brand')[]>([]),[packagingSelected,setPackagingSelected]=useState(false);
 ...
 useEffect(()=>{
  ...
  setSelectedDetails([]);setPackagingSelected(false);
 ...
 const detailFields=data?.manualReview?.detailFields||[];
 const validPackaging=!!role&&!!form&&!!material&&(!data?.current.packaging.length||!!componentKey);
 const canApprove=(!packagingSelected||validPackaging)&&(selectedDetails.length>0||packagingSelected);
 ...
 function submitReview(decision:'approve_selected'|'send_research'|'reject_selected',finishReview=false,rejectField?:'name'|'brand'){
  const permission=data?.manualReview;if(!permission)return;
  const selected=rejectField?[rejectField]:selectedDetails;
  void perform(async()=>{
   const body={requestId:crypto.randomUUID(),expectedRevision:permission.expectedRevision,evidenceFingerprint:permission.evidenceFingerprint,packetDigest:permission.packetDigest,decision,...(decision==='send_research'?{}:{...(selected.length?{fields:selected}:{}),...(!rejectField&&packagingSelected?{component:{key:componentKey||role,role,form,material}}:{}),...(decision==='approve_selected'?{finishReview}: {})})};
   const response=await apiFetch<{saved:boolean}>('/admin/recycling-intelligence/review/'+target.barcode+'/decision',{method:'POST',token:getToken()||undefined,signal:AbortSignal.timeout(20000),body});return response.saved===true;
  },true);
 }
  ...
  <div className='space-y-5 p-4'>
   ...
   {data?<>
    <div className='grid gap-5 sm:grid-cols-2'>
     <section aria-label={word('Product photos','Fotos del producto')} className='space-y-2'>{data.images.length?data.images.map(p=><Photo key={p.url} photo={p} language={language}/>):<div className='flex h-24 items-center justify-center gap-2 rounded bg-stone-100 text-stone-500'><ImageOff size={24}/><p>{word('No photo available','No hay foto disponible')}</p></div>}</section>
     <div className='space-y-5'>
      {!data.current.brand&&data.manualReview&&!detailFields.includes('brand')?<section className='space-y-2'>
      ...
      {!usefulName(data.current.name,target.barcode)&&data.manualReview&&!detailFields.includes('name')?<ProductNameReview key={target.barcode} barcode={target.barcode} language={language} disabled={blocked} onConfirm={name=>decide('confirm_name',name)}/>:null}
      {data.packet?.proposals.length?<section><h3 className='mb-2 font-semibold'>{word('Suggested details','Detalles propuestos')}</h3>{data.packet.proposals.map(p=>{
       const field=p.field as 'name'|'brand',eligible=detailFields.includes(field),current=field==='brand'?data.current.brand:data.current.name;
       const source=data.packet?.sources.find(s=>s.id===p.sourceId);
       return <div key={p.field} className='border-b py-2 text-sm'><label className='flex min-h-11 items-start gap-2'><input type='checkbox' className='mt-1' disabled={blocked||!eligible} checked={selectedDetails.includes(field)} onChange={e=>setSelectedDetails(v=>e.target.checked?[...v,field]:v.filter(f=>f!==field))}/><span className='min-w-0'><span className='text-stone-600'>{field==='brand'?word('Brand','Marca'):word('Name','Nombre')}</span><span className='block break-words'>{current||word('Missing','Falta')} → <strong>{p.value}</strong></span></span></label>
        {eligible?<button disabled={blocked} onClick={()=>submitReview('reject_selected',false,field)} className='min-h-11 text-sm underline'>{word('Keep existing','Mantener actual')}</button>:<p className='text-stone-500'>{current?.trim().toLowerCase()===p.value.trim().toLowerCase()?word('Already saved','Ya guardado'):word('No decision pending','Sin decisión pendiente')}</p>}
        {source?<details className='mt-1'><summary className='min-h-11 cursor-pointer'>{word('Evidence','Evidencia')}</summary><p>{source.identityExcerpt}</p>{safeSource(source.url)?<a href={source.url} target='_blank' rel='noopener noreferrer' className='break-all underline'>{new URL(source.url).hostname}</a>:null}</details>:null}
       </div>;
      })}</section>:null}
      <section><h3 className='mb-3 font-semibold'>{word('Packaging','Envase')}</h3>
       {data.manualReview?<div className='space-y-3'>
        <label className='flex min-h-11 items-center gap-2 text-sm'><input type='checkbox' disabled={blocked||!validPackaging} checked={packagingSelected} onChange={e=>setPackagingSelected(e.target.checked)}/>{word('Include this packaging part','Incluir esta parte del envase')}</label>
        ...
        {packagingConfirmed&&!editing?<p role='status' className='flex items-center gap-2 font-medium text-emerald-800'><Check size={18}/>{word('Packaging confirmed','Envase confirmado')}</p>:null}
      ...
      {data.manualReview?<section className='space-y-2 border-t pt-3'>
       {data.manualReview.disposition?<p role='status' className='text-sm'>{data.manualReview.disposition==='research'?word('Sent for research. Nothing was approved.','Enviado a investigación. No se aprobó nada.'):word('Review finished. Missing information remains unresolved.','Revisión terminada. La información que falta sigue pendiente.')}</p>:null}
       <button disabled={blocked||!canApprove} onClick={()=>submitReview('approve_selected')} className='flex min-h-11 w-full items-center justify-center gap-2 rounded border border-emerald-800 px-3 py-2 text-emerald-900 disabled:opacity-50'><Check size={18}/>{word('Approve selected details','Aprobar detalles seleccionados')}</button>
       {!data.manualReview.disposition?<>{data.manualReview.packetDigest?<button disabled={blocked||!canApprove} onClick={()=>submitReview('approve_selected',true)} className='flex min-h-11 w-full items-center justify-center gap-2 rounded bg-emerald-800 px-3 py-2 text-white disabled:opacity-50'><Check size={18}/>{word('Approve & finish review','Aprobar y terminar revisión')}</button>:null}<button disabled={blocked} onClick={()=>submitReview('send_research')} className='min-h-11 w-full rounded border border-stone-400 px-3 py-2'>{word('Send for research','Enviar a investigación')}</button><p className='text-xs text-stone-500'>{word('Finishing clears your review queue. Missing details stay in research; photos are not published.','Terminar limpia tu cola de revisión. Los datos que faltan pasan a investigación; las fotos no se publican.')}</p></>:null}
      </section>:null}
