...
const copy: Record<DashboardLanguage, {
  ...
  es: {
    ...
    editor: {
      ...
      attachments: "Adjuntos JSON",
      metadata: "Investigación / metadata JSON",
...
function formFromEmail(email: OutreachEmail): OutreachForm {
  return {
    ...
    metadataText: JSON.stringify(email.metadata ?? {}, null, 2),
  };
}
...
export function AdminOutreachWorkspace() {
  ...
  const buildPayload = () => {
    ...
    try {
      ...
      throw new Error(c.invalidJson(c.editor.attachments));
    }

    try {
      metadata = parseJsonField<Record<string, unknown>>(form.metadataText, {});
    } catch {
      throw new Error(c.invalidJson(c.editor.metadata));
    }
  ...
  const disabled = Boolean(action) || !selected || ["sent", "sending"].includes(selected.status);
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(520px,1.25fr)]">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className={`shrink-0 rounded-full border px-2 py-1 text-[11px] font-semibold ${getStatusTone(email.status)}`}>
        ...
        <div className="rounded-xl border border-[var(--gl-hairline)] bg-[var(--gl-paper)] p-5 shadow-sm">
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h2 className="text-xl font-bold text-[var(--gl-ink)]">{c.editor.title}</h2>
              {selected ? (
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <iframe
                  ...
                  srcDoc={form.html_body || "<p style='font-family:sans-serif;color:#718078'>No HTML body.</p>"}
                  sandbox=""
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div>
                  <Label>{c.editor.attachments}</Label>
                  <textarea
                    className="min-h-[180px] w-full rounded-xl border border-[var(--gl-hairline)] bg-white px-3 py-2 font-mono text-xs leading-5 text-[var(--gl-ink)]"
              ...
              {(selected.resend_email_id || selected.error_message) ? (
