...
export function IntelligenceStatistics({data,language,format,onPending,activity}:{data:Outcomes;language:'en'|'es';format:(v:string)=>string;onPending:()=>void;activity?:ReactNode}){
 ...
 return <>
  ...
  <section aria-label={t('Intelligence Contribution','Aportaciones')} className='bg-white p-4'><div className='flex items-center justify-between'><h2 className='font-semibold'>{t('Intelligence Contribution','Aportaciones')}</h2><details className='relative'><summary aria-label={t('About contribution counts','Acerca de las aportaciones')} title={t('About contribution counts','Acerca de las aportaciones')} className='flex h-11 w-11 cursor-pointer list-none items-center justify-center'><Info size={19}/></summary><p className='absolute right-0 z-20 w-72 max-w-[80vw] rounded border bg-white p-3 text-sm shadow'>{t('Open Food Facts counts records with that source, not complete imports. Products may have multiple sources. User photos count distinct products with non-rejected submissions, not uploads. Published counts only current publications linked to user photos, not all user-supported metadata changes. Original import completeness was not recorded.','Open Food Facts cuenta registros con esa fuente, no importaciones completas. Un producto puede tener varias fuentes. Las fotos cuentan productos distintos con envíos no rechazados, no archivos. Publicadas cuenta solo publicaciones vigentes vinculadas a fotos de usuarios, no todos los cambios de datos respaldados por usuarios. No se registró la integridad original al importar.')}</p></details></div>
   ...
   <div className='flex flex-wrap justify-between gap-2 py-3'><dt>{t('User contributions','Aportaciones de usuarios')}</dt><dd><span className='block'>{t('Products with user photos','Productos con fotos de usuarios')}: <strong>{n(data.contributions?.users.products)}</strong></span><span className='mt-1 block'>{t('User photos published','Fotos de usuarios publicadas')}: <strong>{n(data.contributions?.users.improvedProducts)}</strong></span></dd></div></dl>
   <details className='border-t pt-2'><summary className='min-h-11 cursor-pointer py-2 font-medium'>{t('Top contributing users','Usuarios que más contribuyen')}</summary><ul className='divide-y text-sm'>{data.topContributors?.map(u=><li key={u.id} className='flex flex-wrap justify-between gap-2 py-3'><span>{u.name}</span><span>{n(u.products)} {t('products photographed','productos fotografiados')} · {n(u.improvedProducts)} {t('photos published','fotos publicadas')}</span></li>)}</ul>{!data.topContributors?.length?<p className='py-2 text-sm text-stone-500'>{t('No attributed contributions recorded.','No hay aportaciones atribuidas.')}</p>:null}</details>
  </section>
 </>;
}
