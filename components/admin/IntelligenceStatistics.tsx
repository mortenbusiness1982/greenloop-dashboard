 ...
 return <>
  <section aria-label={t('Catalogue progress','Progreso del catálogo')} className='bg-white p-4'>
   <div className='flex items-center justify-between gap-3'><h2 className='font-semibold'>{t('Complete products','Productos completos')}</h2><details className='relative'><summary aria-label={t('About these statistics','Acerca de estas estadísticas')} title={t('About these statistics','Acerca de estas estadísticas')} className='flex h-11 w-11 cursor-pointer list-none items-center justify-center'><Info size={19}/></summary><div className='absolute right-0 z-20 w-72 max-w-[80vw] rounded border bg-white p-3 text-sm shadow'>
    ...
    <p className='mt-2'>{t('Packaging classified is separate. A product can be missing several fields, so missing-field counts overlap.','La clasificación del envase es independiente. Un producto puede carecer de varios datos; los recuentos se solapan.')}</p>
    <p className='mt-2'>{t('Improvements count distinct products with applied changes still in use. Proposals, unchanged saves and rolled-back changes do not count. Latest batch means the latest completed run.','Las mejoras cuentan productos distintos con cambios aplicados aún vigentes. No cuentan propuestas, guardados sin cambios ni cambios revertidos. Último lote significa el último terminado.')}</p>
   </div></details></div>
